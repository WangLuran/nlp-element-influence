# Default pretrained longformer is from https://huggingface.co/potsawee/longformer-large-4096-answering-race
...
def main(args):
    ...
    if model_path == None:
        ...
        tokenizer = LongformerTokenizer.from_pretrained(args.tokenizer_path)
        model = LongformerForMultipleChoice.from_pretrained(args.model_path)
        model.eval().to(device)
    ...
    # input encoding
    def prepare_answering_input(tokenizer, question, options,context,max_seq_length=4096,): 
        c_plus_q   = context + ' ' + tokenizer.bos_token + ' ' + question 
        c_plus_q_4 = [c_plus_q] * len(options)
        tokenized_examples = tokenizer(c_plus_q_4,options,padding="longest",truncation=True,return_overflowing_tokens=False, max_length=max_seq_length,return_tensors="pt")
